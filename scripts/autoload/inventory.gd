...
## pantry_id (a Placement ComponentInstance id) -> {ingredient_id -> {tier -> int}}.
## Ownership of *whether a pantry exists and where* lives in Placement now
## (see docs/design/systems.md, system 4) -- this only tracks what's stored
## inside each one, keyed by component id, so a pantry's storage survives
## being picked up/rearranged/re-placed in Build Mode.
var pantry_stored_ingredients: Dictionary = {}
...
## Total count across every quality tier stored in the pantry. `pantry_id` is
## only checked for existence via Placement -- a pantry that's been picked up
## (in Build Mode storage) still has its stock tracked here, it just can't be
## deposited to/withdrawn from/drawn on for brewing until re-placed.
func pantry_ingredient_count(pantry_id: String, ingredient_id: String) -> int:
	var tiers: Dictionary = (pantry_stored_ingredients.get(pantry_id, {}) as Dictionary).get(ingredient_id, {})
	var total := 0
	for count in tiers.values():
		total += count
	return total
...
func pantry_ingredient_tiers(pantry_id: String, ingredient_id: String) -> Dictionary:
	var pantry_stock: Dictionary = pantry_stored_ingredients.get(pantry_id, {})
	return (pantry_stock.get(ingredient_id, {}) as Dictionary).duplicate()


## Moves stock from the player's carried inventory into the pantry. Fails
## without changing anything if the pantry doesn't exist (Placement) or the
## player doesn't have enough at that tier.
func deposit_to_pantry(pantry_id: String, ingredient_id: String, tier: int, quantity: int) -> bool:
	if Placement.get_component(pantry_id) == null or quantity <= 0:
		return false
	...
	var pantry_stock: Dictionary = pantry_stored_ingredients.get(pantry_id, {})
	var stored_tiers: Dictionary = pantry_stock.get(ingredient_id, {})
	stored_tiers[tier] = stored_tiers.get(tier, 0) + quantity
	pantry_stock[ingredient_id] = stored_tiers
	pantry_stored_ingredients[pantry_id] = pantry_stock
...
func get_save_data() -> Dictionary:
	return {
		...
		"pantry_stored_ingredients": pantry_stored_ingredients.duplicate(true),
	}
...
func load_save_data(data: Dictionary) -> void:
	...
	pantry_stored_ingredients.clear()
	var saved_pantries: Dictionary = data.get("pantry_stored_ingredients", {})
	for pantry_id in saved_pantries:
		pantry_stored_ingredients[pantry_id] = _normalize_stacks(saved_pantries[pantry_id] as Dictionary)
