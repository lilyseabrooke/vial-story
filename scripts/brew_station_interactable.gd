...
## See docs/design/systems.md, system 12. `target_id` is the Brewing station
## id (also its Placement ComponentInstance id) -- this node is only ever
## instantiated once a component exists (Placement.component_placed), so
## there's no "not yet purchased" state left to check.
...
func interact(main: MainScene) -> void:
	var job := Brewing.get_current_job(target_id)
	if job == null:
